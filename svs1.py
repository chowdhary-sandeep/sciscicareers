...
def get_svs(df,dict_auth_N_ord5,min_order=2,max_order=0,approximate=True):
...
#         p = Pool(processes=8)
        if not approximate:
            ...
            pvalues = dict(zip(groups,p.map(p_appr,[tuple([groups[i],sum([dict_auth_N_ord5[x] for x in i])])+tuple([deg_a[ii] for ii in i]) for i in groups])))
        ...
        temp_df['N'] = temp_df['group'].apply(lambda i: sum([dict_auth_N_ord5[x] for x in i]))
#         svh_dfs.append(temp_df.query('fdr'))
        ...
        import pickle
        with open(path_career+'significant_cores3_size'+str(order)+'.pkl', 'wb') as f:
            pickle.dump(significant_cores3_sample, f)    
        with open(path_career+'significant_non_cores3_size'+str(order)+'.pkl', 'wb') as f:
